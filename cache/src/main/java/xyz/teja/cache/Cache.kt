...
class ReactiveCache<T> {
    val book: RxPaperBook = RxPaperBook.with(io())

    fun load(key: String): Single<T> = book.read(key)

    inline fun <reified T> observe(key: String): Flowable<T> =
        book.observe(key, T::class.java, BackpressureStrategy.LATEST)
